...
// Fetch schedules for the logged-in user from the database
// $user_id = $_SESSION['2'];
$user_id = 2;
$stmt = $conn->prepare("SELECT * FROM `schedule_list` WHERE `ID_Perdoruesi` = ?");
...
<body class="bg-light">
    ...
    <?php 
$schedules = $conn->query("SELECT * FROM `schedule_list` WHERE `ID_Perdoruesi` = $user_id");
$sched_res = [];
